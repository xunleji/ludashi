...
public class FlingGalleryView extends ViewGroup {
	...
	// 在系统向该ViewGroup及其各个childView触发onTouchEvent()之前对相关事件进行一次拦�?
	/*
	 * down事件首先会传递到onInterceptTouchEvent()方法
	 * 如果该ViewGroup的onInterceptTouchEvent()在接收到down事件处理完成之后return
	 * false，那么后续的move,
	 * up等事件将继续会先传�?给该ViewGroup，之后才和down事件�?��传�?给最终的目标view的onTouchEvent()处理�?
	 * 如果该ViewGroup的onInterceptTouchEvent()在接收到down事件处理完成之后return
	 * true，那么后续的move,
	 * up等事件将不再传�?给onInterceptTouchEvent()，�?是和down事件�?��传�?给该ViewGroup的onTouchEvent
	 * ()处理，注意，目标view将接收不到任何事件�?
	 * 如果�?���?��处理事件的view的onTouchEvent()返回了false，那么该事件将被传�?至其上一层次的view的onTouchEvent
	 * ()处理�?如果�?���?��处理事件的view
	 * 的onTouchEvent()返回了true，那么后续事件将可以继续传�?给该view的onTouchEvent()处理�?
	 */
//	@Override
...
//	String STR = String.contains("resultStatus={9000}");
	@Override
